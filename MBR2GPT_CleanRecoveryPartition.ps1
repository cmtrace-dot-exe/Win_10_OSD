...
### olearym
### Changelog: 	2018-05-13	- Original Upload
### Changelog: 	2019-03-13	- Increased size of recovery partition & added GPT detection for ID application
############################################################################################################
...
# Set target size for new recovery partition
	#$rePartTargetSize = 525336576
	$rePartTargetSize = 944766976
...
# Resize OS Partition
	Resize-partition -InputObject $osPart -Size $($(Get-PartitionSupportedSize -InputObject $osPart).SizeMax - $rePartTargetSize) -ErrorAction SilentlyContinue
...
# Apply correct ID number to new recovery partition if MBR
	if ($($(get-disk -Number $osPart.DiskNumber).PartitionStyle) -ne "GPT")
	{
